...
from src.utils import ensure_dir, get_device, parse_model_list, set_model_cache_dir, set_seed, write_csv
...
def main():
    ...
    with torch.no_grad():
        for images, labels, relpaths in tqdm(loader, desc="select clean-correct"):
            ...
            preds_by_model = {}
            correct_all = torch.ones_like(labels, dtype=torch.bool)
            for name, model in models.items():
                logits = model(images)
                pred = logits.argmax(dim=1)
                preds_by_model[name] = pred.detach().cpu().tolist()
                correct_all &= pred.eq(labels)

            mask = correct_all.detach().cpu().tolist()
            ...
            for i, ok in enumerate(mask):
                ...
                if args.labels_csv is None:
                    # ImageFolder returns relpath relative to val root. Store relative to data-dir
                    # so later scripts can use data_dir + relpath.
                    full_path = os.path.join(val_root, relpath)
                    relpath_out = os.path.relpath(full_path, data_root).replace("\\", "/")
                else:
                    relpath_out = relpath
                row = {"relpath": relpath_out, "label": labels_cpu[i]}
                for name in surrogate_names:
                    row["pred_" + name] = preds_by_model[name][i]
                rows.append(row)
